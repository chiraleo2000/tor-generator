...
เจ้าหน้าที่ถามกฎหมาย/ระเบียบจัดซื้อจัดจ้างเป็นภาษาไทย ได้คำตอบสตรีม**เป็นย่อหน้าเนื้อหาพร้อมอ้างอิง inline** (ไม่ใช่บันทึกราชการ 6 หัวข้อ) ที่อ้างชิ้นข้อความจาก **คลังกลาง** และ/หรือ **เอกสารของตนเอง**  
...
|----------|---------|-----|
| `chat_rag_top_k` | `config.py` / หน้าตั้งค่า AI | ค่าเริ่ม **๖๔** (env `CHAT_RAG_TOP_K`) |
| `chat_max_context_chunks` | `config.py` / หน้าตั้งค่า AI | ค่าเริ่ม **๘๐** |
| `draft_rag_top_k` | `config.py` / หน้าตั้งค่า AI | ค่าเริ่ม **๒๔** |
| `CHAT_CONTEXT_WINDOW` | `kb_qa.py` | ๑๒๘,๐๐๐ |
| `CHAT_MAX_TOKENS` / `CHAT_MIN_TOKENS` | `llm_tokens.py` | **๘,๑๙๒** / **๐** (ไม่บังคับความยาวขั้นต่ำ) |
| `CHAT_HISTORY_MESSAGES` / `CHAT_HISTORY_CHAR_CAP` | `kb_qa.py` | ๖ ข้อความ / ๑๒,๐๐๐ ตัวอักษรต่อข้อความ |
| งบประมาณบริบท | `kb_qa.py` | ๑๒๘,๐๐๐ − ๘,๑๙๒ − ๔,๐๐๐ ≈ **๑๑๕,๘๐๘** โทเคน |
| อุณหภูมิสตรีม | `chat.py` | ๐.๒ |

ลำดับ:

1. ค้น `hybrid_retrieve` ด้วย `top_k = chat_rag_top_k()` แล้วคละแหล่งด้วย `diversify_chunks` (ผลัดทีละไฟล์ต้นฉบับ) ใส่บริบทได้ถึง `chat_max_context_chunks()` จนกว่างบประมาณหน้าต่าง Gemma จะเต็ม — **ไม่กรอง** `section_relevance` บนคลังกฎหมาย (metadata นี้ไม่มีตอน ingest PDF ดิบ)
...
5. พรอม `KB_QA_SYSTEM`: ขึ้นต้นด้วยคำตอบตรงคำถาม เป็นย่อหน้าไหล แทรกอ้างอิง inline เช่น `(พ.ร.บ. … มาตรา …, หน้า n)` จากชิ้นที่ดึงมา ห้ามโครงบันทึก 6 หัวข้อ ห้ามแต่งมาตรา
...
หน้า `/chat` บอกชัดว่าดึงหลายชิ้นจากคลังแล้วตอบเป็นย่อหน้าเนื้อหาพร้อมอ้างอิง inline (`page-meta` / ข้อความนำหน้าห้อง)
...
ผู้ดูแล: `POST /knowledge-base/sync-mandatory` ซิงก์โฟลเดอร์ข้อมูลดิบแบบเพิ่มไฟล์ (SHA-256 ข้ามซ้ำ ไม่ลบของฉัน) · `POST /knowledge-base/batch-ingest` รันใหม่ทั้งคลัง (๒๐๒, พื้นหลัง) — ปุ่มอยู่ที่**หน้าแอดมินคลังเท่านั้น**
