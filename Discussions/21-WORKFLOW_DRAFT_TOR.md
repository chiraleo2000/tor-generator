...
|--------------|--------|
| **เจ้าหน้าที่ (officer)** | เป็นเจ้าของโครงการ ทำขั้น ๐–๔ แก้ไม่ได้ขณะสถานะกำลังดำเนินการ |
...
|-----:|------|----------------|
| ๐ | เตรียมข้อมูล | อัปโหลดแล้วกดวิเคราะห์ |
| ๑ | ผลวิเคราะห์ | ตารางช่องจากเอกสาร |
| ๒ | สอบถามเพิ่ม | เติมช่องข้อเท็จจริงและกฎ |
...
กล่องยืนยันมีตอน **๐→๑** (วิเคราะห์) **๒→๓** (พร้อมร่าง) **๓→๔** (เข้าทบทวน)  
...
|-----------|-----|----------|
| ไฟล์ (หลายครั้งได้) | `POST .../intake/upload` | สกัดข้อความ เก็บต้นฉบับ ผูก `owner` + `project_id` |
| ข้อความวาง | `POST .../intake/text` | อย่างน้อย **๒๐** ตัวอักษร สูงสุด ๒๐๐,๐๐๐ บันทึกเป็น `ข้อความผู้ใช้.txt` |
| เริ่มวิเคราะห์ | `POST .../intake/analyze` | ต้องมีเนื้อหา กล่อง: *ยืนยันเริ่มวิเคราะห์เอกสาร?* |

ฝั่งลูกข่ายรอวิเคราะห์ได้ถึง **๓๖๐ วินาที** และโพล `GET .../intake/coverage` ขนานกัน ถ้าเอชทีทีพีขาดแต่แบ็กเอนด์จัดช่องเสร็จแล้ว ยังไปขั้นที่ ๑ ได้  
ระหว่างรอแผง `phase0-analyzing` บอกไม่ให้ปิดหน้า

### สิ่งที่วิเคราะห์ทำ

1. จัดช่องจากข้อความทั้งชุด (ฮิวริสติก)
2. ถ้าช่องข้อเท็จจริงยังไม่ครบและเปิด LLM จะเรียกโมเดลใส่ JSON ช่อง แล้วซ่อมคีย์ที่วางผิด
3. บันทึก `slot_map`, `gap_questions`, `analyzed=true` — **`ready_to_compose` ยังเป็นเท็จ**
4. ดึงข้อความกฎจากคลังกลางเข้าช่องที่**ไม่ใช่ข้อเท็จจริง**และยังว่าง (ไม่ทับข้อเท็จจริง) บันทึก `standard_fill_keys` ถ้าคลังไม่พร้อม จะข้ามแล้วยังคืนผลวิเคราะห์
...
**ไม่มีนับถอยหลัง** กด **ไปขั้นที่ ๒** หรือ **ไปสอบถามเพิ่ม** แล้วเข้าขั้นที่ ๒ ทันที
...
งานร่างรันต่อเนื่องในพื้นหลังแม้เบราว์เซอร์หลุด สตรีมส่ง `section_done` เมื่อหมวดบันทึกแล้ว จากนั้น `all_done`
...
**ส่งขออนุมัติ** กดไม่ได้จนกว่า `canSubmitPhase4` เป็นจริง — `filledCount >= 13` (ไม่ล็อกด้วย HITL หรือคะแนน &lt; ๗๐)  
ส่งออก: `POST .../export` รอสถานะ แล้วดาวน์โหลด docx/pdf (TH Sarabun New วันที่ พ.ศ.)
...
| รับข้อมูล | `intake/upload` · `text` · `analyze` · `coverage` · `chat` · `fill-reference(s)` · `open-qa` · `confirm-ready` · `confirm-phase4` |
| ร่าง | `GET/PUT .../sections/{key}` · `draft-section` · `draft-chat/start` · `draft-chat/message` · `draft-chat/status` |
| ทบทวน/ส่งออก | `POST .../review` · `POST .../review/comment` · `GET .../suggestions` · `POST .../submit` · `export` + `download` |
...
| เมนู `/draft` ไม่มีโครงการ | ส่งไปแดชบอร์ด ไม่สร้างโครงการปลอม |
| สตรีมร่างพื้นหลัง | ส่ง `section_done` เมื่อหมวดบันทึก |
...
| ไฟล์ PPTX | ตัวตรวจชนิดไฟล์รับ PDF/Word/รูป/ข้อความ ไม่ใช่พาวเวอร์พอยต์ |
