...
**ตรวจจากโค้ดจริง:** 17 กันยายน 2026 (`app/frontend` + `app/backend` · v0.7.1)  
...
เจ้าหน้าที่สร้างโครงการ **ต้องเลือกหมวดใหญ่ ๗ ประเภท** (ไม่มีค่าเลือกไว้ล่วงหน้า) ส่งเอกสารหรือวางข้อความ ให้ระบบจัดเข้าช่องตาม **Section_Profile** ของประเภทนั้น (หมวดหลัก `s1`–`s13`/`s15`–`s17` + หัวข้อย่อยขอบเขตงานที่แปรตามประเภท ไม่ใช่ชุด `s4.1`–`s4.14` ตายตัว) เติมช่องข้อเท็จจริงในแชท แล้วให้เอไอร่างหมวดตามโปรไฟล์ลงหัวข้อย่อย กดไปทบทวนเมื่อครบหมวดที่โปรไฟล์กำหนด ระบบตรวจกับ พ.ร.บ. การจัดซื้อจัดจ้าง กฎระเบียบ และเอกสารขั้นที่ ๐ ของโครงการนี้ จากนั้นส่งขออนุมัติและส่งออกเวิร์ด/พีดีเอฟ

รายละเอียดชุดหัวข้อต่อประเภท: [37-TOR-PROFILES-BY-TYPE.md](37-TOR-PROFILES-BY-TYPE.md)

โครงเอกสาร**แปรตามประเภทงาน** ความครบของหมวดใช้ `isSectionFilled`: มีเนื้อหาหลัก หรือมีหัวข้อย่อยหมวด ๔ ที่เติมแล้ว
...
ฟอร์มสร้าง: ชื่อโครงการ หน่วยงาน วงเงิน (ตัวเลข ASCII) **หมวดใหญ่ประเภทการจัดซื้อจัดจ้าง ๗ ค่า (บังคับเลือก ไม่มีค่าเริ่มต้น)** แม่แบบ (ถ้ามี)  
ห้ามเปลี่ยนหมวดใหญ่หลังเข้าขั้นที่ ๓
...
3. บันทึก `slot_map`, `gap_questions`, `analyzed=true` แล้ว **commit หลัง LLM เสร็จ** (ไม่ค้าง idle-in-transaction ระหว่างวิเคราะห์) — **`ready_to_compose` ยังเป็นเท็จ**
4. **ไม่ดึงคลังกฎหมายอัตโนมัติตอนวิเคราะห์แล้ว** (`standard_fill_keys` คืนว่าง) — กด **ใช้มาตรฐานกลางเติมช่องว่าง** ในขั้นที่ ๒ (`POST .../intake/fill-references`) ซึ่งต้องมี `analyzed=true` ในฐานข้อมูลแล้ว

ไปขั้นที่ ๑ เมื่อ `analyzed === true` และตารางความครบยาวกว่า ๐

### ช่องตาม Section_Profile

ชุดช่องไม่ใช่ ๒๗ แถวตายตัว — ดู [37](37-TOR-PROFILES-BY-TYPE.md)  
จัดซื้อครุภัณฑ์ / ก่อสร้าง / จ้างเหมาเอกสาร **ไม่มี** หัวข้อย่อยระบบงานปัจจุบัน

**ช่องข้อเท็จจริงบังคับ** (ตัวอย่างโปรไฟล์ `buy_goods` — กฎหมายอย่างเดียวเติมไม่ได้):
...
| `s6` | วงเงินงบประมาณ · ราคากลาง · วิธีจัดซื้อ |
| `s7` | สถานที่ดำเนินการ |
| `items` (alias `s4.1`) | รายการพัสดุที่จัดซื้อและจำนวน |
...
แสดงสรุปข้อเท็จจริงที่เติมแล้ว ตารางตามโปรไฟล์ประเภทงาน และคำถามช่องว่าง  
...
|------|----------------------------------|
| `s1` | `history` ประวัติ · `problems` ปัญหา · `policy` นโยบาย · `workKind` ประเภทงาน |
| `s2` | `mainObj` วัตถุประสงค์หลัก · `users` กลุ่มผู้ใช้ · `kpi` ตัวชี้วัด |
| `s3` | `general` e-GP/ผู้ทิ้งงาน · `paidup` ทุน/มูลค่าสุทธิ · `experience` ผลงานย้อนหลัง · `specialist` OEM/ที่ปรึกษา/ทีม |
| `s4` | ร่างลงหัวข้อย่อยตาม Section_Profile ของประเภทงาน (เช่น `items` / `functional` / `cm`) ไม่ใช่ชุด `s4.1`–`s4.14` ตายตัว |
| `s5` | `timelineRange` วันเริ่ม–สิ้นสุด · `milestones` งวดงานหลัก |
| `s6` | `budgetAmount` วงเงิน · `budgetSource` ที่มา · `announcedPrice` ราคากลาง · `priceBasis` วิธีคำนวณ · `procurementMethod` วิธีจัดซื้อ |
| `s7` | `location` สถานที่ |
| `s8` | `installments` จำนวนงวด · `paymentTerms` เงื่อนไขเบิกจ่าย · `retention` ประกันผลงาน |
| `s9` | `warranty` รับประกัน |
| `s10` | `penalty` ค่าปรับส่งมอบ · `slaPenalty` ค่าปรับ SLA |
| `s11` | `evalMethod` วิธีประเมิน · `evalWeight` สัดส่วน · `passingGrade` คะแนนผ่าน |
| `s12` | `docs` รายการเอกสาร · `socTable` ตารางเปรียบเทียบข้อกำหนด |
| `s13` | `other` เงื่อนไขอื่น · `ipCopyright` ลิขสิทธิ์ · `nda` ความลับ · `reservations` ข้อสงวนสิทธิ์ · `responsibleUnit` ผู้รับผิดชอบ |

ตารางในเนื้อหาจะเป็นตารางจริงในไฟล์ส่งออก

ปุ่มในแชทต่อหมวด: **ยอมรับ** · **แก้ไข** · **ร่างใหม่** (ส่ง `section_key` ไปกับข้อความ)  
ปุ่มบนการ์ด: **ร่างด้วยระบบอัจฉริยะ** (`POST .../draft-section` กราฟ LangGraph เกณฑ์ ๗๐ ลองใหม่ได้ไม่เกิน ๓ · **Rule Engine โฟกัสหมวดที่ร่าง** ด้วย `_focus_section` จึงไม่ให้คะแนน 0 เพราะหมวดอื่นยังว่าง) และ **บันทึกหมวดนี้**  
...
เกตไปขั้นที่ ๔ ฝั่ง UI: `canLeavePhase3(allDrafted)` — ครบหมวดตาม Section_Profile (`isSectionFilled`)  
...
แผงขั้นที่ ๔: แถบความครบ · พรีวิวร่างรวม · ความเห็นรวม · คะแนนกฎ · ข้อค้นพบสองกลุ่ม (ก ผิดกฎหมาย/ระเบียบ พร้อม `legal_basis` จาก RAG · ข ความเสี่ยงจากความผิดปกติ) · ข้อเสนอแนะเอไอ · อัปโหลด **ข้อกำหนดเพิ่มเติม** (`GET/POST/DELETE .../review/requirements` รับ PDF/DOCX/TXT เข้าบริบท ReviewAgent ไม่เข้าน้ำหนักกฎ) · แชทรีวิว · **ย้อนกลับไปแก้ไข** · **ตรวจด้วยกฎระเบียบ** · **ส่งขออนุมัติ**

ค้นกฎหมายตอนร่างใช้คลังกลางชุดเดียวกับแชท (`draft_rag_top_k` ค่าเริ่ม ๒๔) **ไม่กรอง** `section_relevance` แนวความครบถ้วนมาจาก few-shot ใน `tor_draft_hints.py` (โครงตัวอย่างสามฉบับ) **ไม่ ingest PDF ใน `documents/sources/ตัวอย่าง/` เข้าคลังกฎหมาย**  
การ์ดแยก **เผยแพร่**: ส่งออกเวิร์ด/พีดีเอฟ

**ส่งขออนุมัติ** กดไม่ได้จนกว่าหมวดทองตาม `Section_Profile` ของประเภทงานจะมีเนื้อหา — ฝั่งหน้าจอนับหมวดที่เติมแล้วตามโปรไฟล์ (ไม่ใช่ ๑๓ หมวดตายตัว)  
ฝั่งเซิร์ฟเวอร์ `POST .../submit` เรียก `missing_submit_sections()` ตามหัวข้อที่ `required` ในโปรไฟล์ทอง (รวมสำรองข้อมูล/เอกสารส่งมอบ/เงินประกันผลงาน หรือเงื่อนไขยื่นข้อเสนอตามประเภท) และปฏิเสธหมวดที่ว่างหลัง sanitize — `officer_can_submit` เป็นเกตสถานะ (`draft`/`rejected`/บางกรณี `archived`) ควบคู่กัน  
