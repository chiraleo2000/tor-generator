...
/**
 * Optional evidence capture only — excluded from `npm run test:e2e` via playwright.config
 * `testIgnore`. Run with: `npm run test:e2e:reports` after serving:
 * - backend htmlcov on :8765
 * - frontend coverage on :8766
 * - playwright-report on :8767
 */
