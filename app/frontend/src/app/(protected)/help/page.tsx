    ...
    <div data-testid="help-page">
      ...
      {tab === "draft" ? (
        <GuideBlock title="กระบวนการร่าง 5 Phase">
          ...
          />
          <ol>
            <li>Phase 0–1 เป็นแชทโครงการ — วางข้อความหรืออัปโหลดหลายไฟล์ก่อน จึงจะไป Phase 2</li>
            ...
            <li>Phase 3 กดรัน Rule Engine — ดู findings และข้อเสนอแนะจาก ReviewAgent แล้วส่งขออนุมัติเมื่อครบ 13 หมวดและยืนยัน HITL</li>
            <li>Phase 4 ส่งออก Word/PDF — ถ้าสร้างเอกสารล้มเหลว ระบบแสดงข้อความ ไม่ค้างเงียบ</li>
          </ol>
          <table>
            <thead>
              <tr>
      ...
      {tab === "chat" ? (
        <GuideBlock title="ถาม-ตอบ">
          <ol>
            ...
            <li>ระหว่างรอคำตอบมีจุดพิมพ์ และข้อความแสดงเวลาเมื่อ API ส่งมา</li>
      ...
      {tab === "review" ? (
        <GuideBlock title="ตรวจสอบ TOR">
          <p>
            มีสองเส้นทาง: ในโครงการที่ Phase 3 (Rule Engine + ReviewAgent) และหน้า /review สำหรับไฟล์ภายนอก (Rule Engine)
            อัปโหลดไฟล์ที่มีอยู่แล้ว ระบบอ้างอิงกฎหมายบังคับเสมอ เทียบเคียงไฟล์อื่นด้วยความคล้ายของข้อความ
            คะแนนต่ำกว่า 70 แสดงว่ายังไม่ผ่านเกณฑ์เบื้องต้น
          </p>
        </GuideBlock>
      ) : null}
      {tab === "admin" ? (
        <GuideBlock title="ผู้ดูแลระบบ">
          ...
          <p>
            แชทและฝังเวกเตอร์เลือกอิสระในทุกโหมด — โหมด ผสม แนะนำเมื่อใช้คนละแหล่ง
            ตัวอย่าง: Claude (Anthropic) + EmbeddingGemma ใน LM Studio · Claude + ฝังเวกเตอร์ OpenAI (ต้อง seed คลังใหม่)
            คลาวด์: Anthropic / OpenAI / Gemini / Bedrock / Azure Foundry / OpenAI-compatible
            บันทึกแล้วมีผลทันที คีย์ไม่โชว์เต็ม · ทดสอบการเชื่อมต่อ ยิงทั้งแชทและฝังเวกเตอร์
          </p>
        </GuideBlock>
      ) : null}
      {tab === "faq" ? (
