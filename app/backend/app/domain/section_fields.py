...
def field_prompt_block(section_key: str) -> str:
    ...
    if not rows:
        return ""
    lines = [
        "ครอบคลุมสาระต่อไปนี้ในเนื้อหาเอกสารราชการ",
        "ห้ามพิมพ์รหัสภาษาอังกฤษ ป้ายช่องข้อมูล หรือข้อความในวงเล็บคำสั่งเป็นหัวข้อในร่าง",
        "ห้ามขึ้นต้นด้วย ### ตามด้วยรหัส เช่น history problems workKind",
        "ห้ามพิมพ์เลขนำหน้าชื่อหมวดหรือหัวข้อย่อย — ระบบส่งออกเป็นผู้ใส่หัวข้อ",
        "หมวดความเป็นมาเขียนเล่าเรื่องต่อเนื่องไม่มีหัวข้อย่อย และปิดท้ายด้วยจึงมีความจำเป็น",
    ]
    ...
    if hint:
        lines.append(f"แนวทางความครบถ้วนจากตัวอย่าง (ใช้เป็นสาระ ไม่พิมพ์เป็นหัวข้อ): {hint}")
    return "\n".join(lines)
