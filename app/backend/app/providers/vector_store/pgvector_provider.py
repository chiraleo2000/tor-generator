...
def _split_acl_filter(
    filter: dict | None,
) -> tuple[dict[str, Any] | None, str | None, str | None]:
    """Strip ACL keys from a JSONB filter. Mine without owner is handled by the caller."""
    if not filter:
    ...
    return (jsonb_filter or None), search_scope, owner_user_id
...
class PgVectorProvider(VectorStoreProvider):
    ...
    async def search(
        ...
        jsonb_filter, search_scope, owner_user_id = _split_acl_filter(filter)
        if search_scope == "mine" and not owner_user_id:
            return []
        ...
        if search_scope or owner_user_id:
            ...
            elif search_scope == "global" or not owner_user_id:
